...
- **For every test the diff adds or changes, name the mutation that turns it red.** A test that cannot fail is not coverage; it is decoration that reads exactly like the real thing in a green run, and it is worse than no test because it is counted as one. Name the change to the production code that would make it fail — "return the cached value instead of refetching", "compare with `<` where the boundary needs `<=`" — concretely enough that someone could make it. Where you cannot name one, say so and reject **that test**: the fix is a sharper assertion, not more code. Five tests that could not fail shipped in a single milestone before anyone asked the question, and every one of them was found by asking it.
...
<!-- nina:slot money.2 -->
<!-- nina:slot db.5 -->
3. **patterns-and-scope** — the spec's file list, ranges and premises checked against the tree
   ...
   test the diff adds or changes, the mutation that turns it red, rejecting any test that has none;
...
<!-- nina:slot money.5 -->
- **Tests:** for every test added or changed, the mutation that would turn it red. A test you cannot name one for is reported as such rather than counted.
