...
## When you run
- After the LAST sub-step of a milestone (a numbered set like `6.*`, or a phase like `{{PROVIDER_PKG}}`) passes qa, BEFORE the milestone is declared complete.
...
<!-- nina:slot money.2 -->
<!-- nina:slot edge-cf.2 -->
9. **Error handling / info disclosure** — the `{ok,error}` envelope must not leak stack traces, internal IDs that aid enumeration, or library internals to the client.
10. **Rate-limiting / abuse / DoS** — note where an unauthenticated or cheap endpoint (magic-link request, dev routes) lacks throttling and could be abused (especially anything that sends email or writes rows).
