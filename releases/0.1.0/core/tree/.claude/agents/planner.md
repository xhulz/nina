...
- **Mixed concerns**: schema + service + route + UI in one breath; or match + split + payout logic spanning `{{CORE_PKG}}` and the Durable Object in one breath.
- **Multiple distinct test surfaces**: e.g., pure unit + Miniflare integration + {{PROVIDER}} contract-test conformance in the same subtask.
- **Multiple security invariants** that the reviewer would need to verify simultaneously (e.g., "`userId` scoping AND secrets handling AND payout idempotency" in one diff).
...
> money / Prisma / auth / {{PROVIDER}}, or lands in a different package. Otherwise it is a **sibling
