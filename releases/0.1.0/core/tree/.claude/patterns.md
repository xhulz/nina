...
- **Zod** for runtime validation at all trust boundaries (API ingress, env loading, queue payload, DO RPC, webhook body, {{PROVIDER}} responses).
...
- **Data Objects:** Prisma client, per-account DO, R2, Queue producers, {{PROVIDER}} client (via `{{PROVIDER_PKG_NAME}}`).
...
- Reviewer verifies: typecheck clean, Biome clean, dba approved (if Prisma touched), integration-tester approved (if external-service / {{PROVIDER}} touched), no secret leaked, no PII in logs or tests.
