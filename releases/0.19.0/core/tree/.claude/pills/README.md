...
graduating it is a request: `nina learn --graduate <pill>` writes one under `.nina/requests/`,
carrying the pinned version, the layer the rule belongs in and the lesson in this project's words,
and the harness maintainer reads every open one with `nina requests`. The pill stays active until
the release carrying the rule is installed — retiring it earlier drops the lesson in the gap.
