...
3. **Build-time env.** Vite inlines `VITE_*` at build time; `[vars]` in `wrangler.toml` never reach a Pages build. Verify every `VITE_*` the app reads is present in the build environment — a missing one ships a blank page, not an error.
