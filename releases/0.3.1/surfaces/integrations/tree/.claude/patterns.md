...
An **integration** is a dependency whose behavior this project does not define. Each one is declared in `.harness/profile.json` with a `kind`, and the kind decides what counts as evidence:
...
3. **The profile entry** — `slug`, `name`, `kind`, `boundary` and `skill` in `.harness/profile.json`. This is what makes the integration visible to the gates; an integration that is not declared is not gated.
