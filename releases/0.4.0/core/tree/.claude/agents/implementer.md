...
- A short diff summary for the reviewer, including an explicit note if Prisma, or any integration boundary, was touched.
...
## Handoff
Diff + summary → **reviewer**. **Cap your final message at ~250 words** unless flagging a complex deviation. The first line of your summary must state whether Prisma, or any integration boundary, was touched (so the reviewer knows which gates to run). Include in the summary (one short bullet each):
