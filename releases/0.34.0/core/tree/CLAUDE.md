...
1. **Classify the task** using the table below *before* touching any file — and before a session's first dispatch, read `.claude/graph.md` and `.claude/router.md`.
...
| **Spike** — an experiment that answers a question about a service, a library or the data before anything is designed on it | **architect (spike plan) → implementer → reviewer** — no qa, devops or secops; `.claude/router.md` § *A spike answers one question* |
...
**Sizes, and what each decides.** ≤2 files and no critical path: the light chain. Past 4 files or ~400 new lines, the planner decomposes a subtask. One implementer run writes at most {{STEP_FILES}} files, and the architect splits a longer list into steps. A `ONE-SPEC` group stays under ~400 lines of diff. Past ~200 lines, the reviewer fans out by dimension. Each size decides only its own thing.
...
18. **Only the orchestrator commits, and no stage moves the checkout.** A stage reads history (`git diff`, `log`, `show`, `status`, `blame`) and builds or tests another revision in a `git worktree` it removes afterwards. It never stashes, checks out, switches, resets, restores, cleans, commits, merges or rebases in the tree it was dispatched into: the work in flight lives there, uncommitted, and a stash or a checkout takes it from every stage beside it.
19. **Every turn re-reads the whole context, so take each thing in once.** Tool calls that do not depend on each other go out in one message. A file you wrote or edited is in your context already: read it again only once something else changed it. Find before you open (`grep -l`, `grep -c`), then read the range you need; output longer than a screen goes to a file you grep.<!-- nina:why --> In the project this was learned in, 99% of an implementer's ~350 turns sent a single call, and runs re-read files they had just written 534 times.<!-- /nina:why -->
