...
<!-- nina:slot db.5 -->
2. **tenant-and-privacy** — `userId` in every app query ({{AUTH_LIB}} tables are the only exemption);
   no PII or secrets in logs (CPF/CNPJ, PIX keys, account numbers, KYC bytes); no cross-tenant read
