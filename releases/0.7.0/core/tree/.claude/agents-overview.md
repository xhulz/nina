...
than three: `reviewer`, plus `dba` when the database was touched, plus `integration-tester` when an
external service was. On a large diff the reviewer itself fans out by dimension — one per axis of risk,
