...
difference is the `nina:generated` marker, not the directory. `nina where <path>` answers it for any
path, and prints the same sentence that file's own header carries. The question that decides it:
**is this a lesson of this project, or a rule for any project?**
