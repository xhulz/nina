<!-- nina:slot db.1 -->
| `prisma-cli` | **before ANY `prisma` CLI command** — `validate`, `format`, `migrate dev`, `migrate diff`, `migrate deploy`. Flags differ between versions and one of them (`migrate diff --shadow-database-url` against a live database) already wiped dev and staging on 2026-09-16 |
| `prisma-client-api` | auditing a new or changed query — filters, operators, `$transaction` semantics |

<!-- nina:slot db.2 -->
Neither skill covers **Prisma Accelerate** — `cacheStrategy`, edge-client behavior and the
no-cache-on-money-reads rule live in `.claude/integrations/prisma-accelerate.md`. The skill does not
replace that file.
...
2. **Migration diff.** Run `prisma migrate diff` in its **offline, file-only form** and read the generated SQL, not just the Prisma delta:
   ...
   ⛔ **NEVER pass `--shadow-database-url`, `--from-url`, `--to-url`, or `--from-schema-datasource` pointing at a real database — most of all not `DATABASE_URL` from `{{API_DIR}}/{{SECRETS_LOCAL}}`.** Prisma **executes** the entire migration history against whatever database those flags name, which **DESTROYS ALL DATA** in it. A shadow database must be a disposable, empty database and nothing else. This is not hypothetical: on 2026-09-16 a dba run wiped the whole shared dev/staging database this exact way — 79 users, 75 escrow accounts and every KYC submission — because the command completed with a reassuring "empty migration" result and no error. If the offline form above cannot answer your question, **STOP and report that to the orchestrator** instead of reaching for a database URL.
...
<!-- nina:slot db.9 -->
4. **Index coverage.** Every new query pattern (`WHERE`, `ORDER BY`, `JOIN`, `aggregate`) must be backed by an index. Use `@@index` in schema. Reject queries that would table-scan. The most-used compound index is `(userId, <other>)` or, for account-keyed models (`{{ENTRY_MODEL}}`, `{{PAYOUT_MODEL}}`), `(accountId, <other>)`; verify the leading column matches the query.
