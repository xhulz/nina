...
- Before running, **sanity-check that no stray vitest/workerd is alive:**
  ```bash
  ps aux | grep -iE "vitest|workerd" | grep -v grep | awk '{print $2}' | xargs -r kill -9 2>/dev/null
  ```
...
- Run vitest for each affected package **sequentially**, never in parallel. Use:
  ```bash
  cd <package-path> && pnpm exec vitest run [--reporter=verbose]
  ```
...
- For each test run, capture: pass count, fail count, and (when failing) the failing test names with their assertion messages.
- After the run completes (pass or fail), verify no vitest/workerd workers are lingering:
  ```bash
  ps aux | grep -iE "vitest|workerd" | grep -v grep | head -5
  ```
...
- Run tests in parallel across packages. The configs enforce single-worker; running multiple invocations concurrently spawns multiple workerd processes (~2 GB each).
- Run `pnpm test` at the workspace root unless the diff truly justifies it. The root script chains turbo across all packages.
- Ignore lingering vitest/workerd processes. Always kill them at start AND exit.
...
- **Always** kill any leftover vitest+workerd processes at start AND end of your dispatch.
- **Monitor**: if you suspect memory bloat, run `ps aux | grep -iE "vitest|workerd" | awk '{print $2, $6}'` and check resident memory.
...
<!-- nina:slot project.5 pre-existing-failure-lesson -->
...
- Tests fail because of an environment issue (DB unavailable, etc.) → back to **integration-tester** if applicable, else **implementer** for a deterministic mock.
