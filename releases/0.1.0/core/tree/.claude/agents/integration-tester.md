...
- The architect's spec. Specifically: the **External library premises** section the architect was required to include. For a real service, each premise should carry a citation pointing into `node_modules/.pnpm/<lib>@<version>/.../<file>:<line>` (or into a `.claude/integrations/<lib>.md` premise). For {{PROVIDER}}, the spec references the **contract-test cases** the mock must satisfy (the contract is ours, not in `node_modules`).
- The implementer's diff and test file additions.

If the architect's spec has NO External-library-premises section (or, for {{PROVIDER}}, NO contract-case references) but the diff touches a gated surface → halt and reject upstream. The architect must amend the spec with citations / contract-case references before you can validate them.

## Inputs

- Architect spec (with External-library-premises section, or {{PROVIDER}} contract-case references).
...
- **Premises / contract cases verified:** list each premise (real service) or contract case ({{PROVIDER}} — say which implementation) from the spec, mark `verified` / `falsified` / `unverifiable` with evidence (citation OR command output).
- **New premises discovered:** any runtime behavior or contract refinement you observed that wasn't in `.claude/integrations/<lib>.md` (or `{{PROVIDER_DOC}}`) and isn't in the spec. Each gets PR-ready text appended to your report for the relevant integration doc.
...
- **Drive the flow end-to-end where possible.** For {{AUTH_LIB}}: sign-up / magic-link → assert session shape → assert DB row shape, scoped by `userId`. For a Durable Object payout path: enqueue → assert the per-account DO serializes and de-dupes → assert binding I/O. Don't stop at the first happy-path call.
...
- Approve without running the real flow (real services) or the contract-test suite against BOTH {{PROVIDER}} implementations. "Unit tests pass" is the reviewer's check, not yours.
- Use mocks for a **real** external dependency you're validating. Mocks at this stage defeat the purpose. For {{PROVIDER}} the mock is *one of two* things under test — it is never the only one.
...
4. **New premises discovered:** verbatim text to append to `.claude/integrations/<lib>.md` (or `{{PROVIDER_DOC}}`).
