...
minutes to hours. A different issue on the same edge starts its own count. The cap is an instruction to the orchestrator,
not something the harness can enforce — see `router.md` for how the count is kept.
...
- `qa` → `implementer` on `FAIL` — a test fails, or a "pre-existing" failure does not reproduce · max 2
