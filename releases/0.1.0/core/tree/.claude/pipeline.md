...
`dba`, `integration-tester` and `reviewer` are all read-only, so **dispatch them together in one
...
- **Output:** technical spec — files to touch, function signatures, data flow, edge cases, tests to add, patterns to follow. Marks **"DBA REQUIRED"** if Prisma will be touched. Marks **"INTEGRATION-TESTER REQUIRED"** if an external-service surface will be touched, AND includes an **External library premises** section: every behavior the implementation depends on, each with a `node_modules/.pnpm/<lib>@<version>/.../<file>:<line>` citation OR a `P<n>` reference into `.claude/integrations/<lib>.md`. For {{PROVIDER}}, the spec references the relevant contract-test cases both implementations must satisfy. No premise without a citation.
- **Tools:** Read, Write, Edit, Grep, Glob, WebSearch, WebFetch. **No edits except spec files + `.claude/integrations/**`.**
- **Exit criteria:** implementer can code without re-planning AND integration-tester can verify each premise / contract case without re-deriving it.
...
- **Exit criteria:** typecheck/lint/build pass; no scope creep; if Prisma touched, says so on first line; if an external service surface touched, says so.
<!-- nina:slot db.1 -->
...
- Runs vitest **once**, per affected package, **sequentially** (configs enforce single-fork). Kills stray vitest/workerd at start and end. See `.claude/agents/qa.md`.
...
- **Reviewer rejects for design issue** → loop back to architect.
- **Architect realizes plan is wrong** → loop back to planner.
<!-- nina:slot db.3 -->
<!-- nina:slot external-api.3 -->
<!-- nina:slot money.2 -->
- **Implementer hits an unknown unknown** → stop, escalate. Don't guess.
