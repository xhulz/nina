...
Reviewer rejects any architect spec that touches Workers / DO / Queues / wrangler config without citing which skill it consulted, OR explaining why the skill was not needed. Same standard as the `node_modules:<line>` premise rule (#12 below): retrieval beats recall.
...
- Subagent role specs → `.claude/agents/` (dispatchable via Agent tool) — **generated.**
  ...
  declares in `.harness/profile.json`, and its own overrides in `.harness/project/tree/`.
...
> The pipeline is **proportional, not one-size-fits-all.** Money-movement, Prisma, auth, {{PROVIDER}}, and substantial multi-file features get the full chain. Small read-only / display / UI-copy changes get a light chain — or a direct edit. **Under-gating a money/Prisma/auth/{{PROVIDER}} change is a protocol violation. Over-gating a one-file label or list change wastes hours — that is also a failure.**
...
7. **Right-size before dispatching.** Match the chain to blast radius (see table). A change that touches no money path, no Prisma, no auth/{{PROVIDER}} surface, and ≤2 files takes the light chain — or a direct edit when a subagent adds nothing (a label, a copy tweak, a list render). Do NOT run planner/architect/secops on that. The money/Prisma/auth/{{PROVIDER}} gates below are never optional.
...
| **Small read-only / display / UI-copy change** (≤2 files, no money path, no Prisma, no auth/{{PROVIDER}}) | **implementer → reviewer** — or a direct edit if a subagent adds no value; qa runs ONLY the affected test file. No planner, no secops. |
...
| **ANY step that changes a deployed surface** (API, frontend, schema, wrangler config, secrets, bindings) | **+ devops after qa** — preview deploy + smoke; prod only on an explicit go |
<!-- nina:slot frontend.2 -->
| **END of a spec-SET / milestone** (a numbered set like `6.*`, or a phase like `{{PROVIDER_PKG}}`) | **+ secops after the last sub-step's qa** — milestone security gate; `SECURE` required before the milestone is declared done |
...
→ If the doubt is whether a **money / Prisma / auth / {{PROVIDER}}** surface is touched → treat it as the heavier shape and gate it. If the task is clearly none of those and the only question is "how much ceremony" → take the **lighter** chain; an extra planner/secops pass on a display change costs hours, not minutes.
...
- **secops** → **milestone security gate.** Runs once at the END of a completed spec-SET (a numbered milestone like `6.*`, or a phase like `{{PROVIDER_PKG}}`), after the last sub-step's qa — NOT per sub-step. Audits as an attacker + LGPD auditor (auth/session, tenant isolation, secrets/config exposure, PII leakage, money-movement abuse & idempotency seams, injection/SSRF, dependency & binding posture). **Scoped to the milestone diff by default; a whole-tree sweep runs only on explicit request** (it surfaces pre-existing debt mid-task → scope-creep). Read-only; CRITICAL/HIGH findings in the milestone's own surface BLOCK it and loop back until re-audited.

<!-- nina:slot project.6 -->

## Hard rules (non-negotiable)

1. **Run the subagent chain proportional to the task (see the pipeline table).** Under-gating a money-movement, Prisma, auth, or {{PROVIDER}} change is a protocol violation — those gates are never skipped, and when uncertain whether such a surface is touched, gate it. But small read-only / display / UI-copy changes (≤2 files, none of those surfaces) take the light chain or a direct edit — over-gating them wastes hours and is also a failure.
<!-- nina:slot db.5 -->
<!-- nina:slot money.1 -->
<!-- nina:slot money.2 -->
<!-- nina:slot money.3 -->
