...
- **A spike** (`.claude/router.md` § *A spike answers one question*) is reviewed for one thing: that it measures what it says. Walk each check's mutation through the data it reads, make every branch fail closed, and trace each number it reports to what it read. Ask none of what a product change owes.
