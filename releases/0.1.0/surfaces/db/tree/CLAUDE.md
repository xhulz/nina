...
<!-- nina:slot db.2 -->
| Write or audit a Prisma query (architect, implementer, dba, reviewer) | `prisma-client-api` | filters, operators, `$transaction` semantics. **Accelerate is NOT covered** — `cacheStrategy` lives in `.claude/integrations/prisma-accelerate.md` |
...
<!-- nina:slot db.6 -->
6. **Every hot Prisma read defines a `cacheStrategy`** (Accelerate `ttl` / `swr`). Implicit cache is a bug. **Never cache a read that feeds a conciliation or payout write** — a stale entry or balance contaminates real money movement. User-scoped invalidation on every write.
